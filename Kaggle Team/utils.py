...
def get_contexts():
    SIM_MODEL = '/kaggle/input/sentence-transformer-model/gte-small'
    DEVICE = 0
    MAX_LENGTH = 384
    BATCH_SIZE = 32
    ...
    trn = pd.read_csv(
        "/kaggle/input/kaggle-llm-science-exam/test.csv").drop("id", 1)
    ...
    sentence_index = read_index(
        "/kaggle/input/wikipedia-faiss-index/merged.index")
    ...
    for file in tqdm(wikipedia_file_data.file.unique(), total=len(wikipedia_file_data.file.unique())):
        _id = [str(i) for i in wikipedia_file_data[wikipedia_file_data['file']
                                                   == file]['id'].tolist()]
        _df = pd.read_parquet(f"{WIKI_PATH}/{file}",
    ...
    wiki_text_data = pd.concat(
        wiki_text_data).drop_duplicates().reset_index(drop=True)
    _ = gc.collect()
    ...
    # Combine all answers
    trn['answer_all'] = trn.apply(lambda x: " ".join(
...
def generate_openbook_output():
    ...
    test_df["prompt"] = test_df["context"].apply(
        lambda x: x[:2200]) + " #### " + test_df["prompt"]
    test_df['answer'] = 'A'
    model_dir = "/kaggle/input/run-sft-awp-0-1-ls-dropout-4e6/checkpoint-5600"
    ...
    option_to_index = {option: index for option,
                       index in zip(options, indices)}
    index_to_option = {index: option for option,
                       index in zip(options, indices)}
    ...
    tokenized_test_dataset = Dataset.from_pandas(test_df[['id', 'prompt', 'A', 'B', 'C', 'D', 'E', 'answer']].drop(
        columns=['id'])).map(preprocess, remove_columns=['prompt', 'A', 'B', 'C', 'D', 'E', 'answer'])
    ...
    # model_dir = "/kaggle/input/singlegpu-93k-best-parameter/checkpoint-4700"
    # tokenizer = AutoTokenizer.from_pretrained(model_dir)
    # model = AutoModelForMultipleChoice.from_pretrained(model_dir).cuda()
    # model.eval()
    ...
    # test_predictions = softmax(test_predictions) * 2.0 + softmax(
    #     test_predictions2) * 1.0 + softmax(test_predictions3) * 1.0
